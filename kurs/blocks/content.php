<?php 
  ...
  //				echo "<br>$tmp[0]";
  if ($by>=$tmp[0]) {$by=$tmp[0]-$b ;}
  if ($_SESSION['lang']=='en')
  {
    $querynews="SELECT id,nameen,descriptionen,author,date FROM news ORDER BY date DESC LIMIT $by, $b";
  }
  else
  {
  ...
  if ($by<1)
    printf ("<p><span class='right'><a href='index.php?by=%s'>&#8594;</a></span></p>",$by+$b);
  elseif ($by>=($tmp[0]-$b))
    printf ("<p><span class='left'><a href='index.php?by=%s'>&#8592;</a></span> </p>",$by-$b);
  else
