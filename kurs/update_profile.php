	...
	<div id="wrapper"> 
		...
		<div id="content">
            ...
            //else {$texterrror='<p>Please fill out both password fields.</p>'. ' ' .$texterrror;}
				if ($texterrror!="")
				{/*echo $texterrror;echo "<html><head>
            ...
            exit ();*/
				...
				$monthB = $_POST[monthB];
				$dayB = $_POST[dayB];
				if (!preg_match('|([a-z0-9_\.\-]{1,20})@([a-z0-9\.\-]{1,20})\.([a-z]{2,4})|', $email))
				{/*echo "<html><head>
            ...
            echo "<p>".$arrayup[mail_error]."</p>";exit;*/
				...
				{/*echo $arrayup[pass_error];echo "<html><head>
				...
				else
				{
				  ...
				  && ($_FILES["file"]["size"] < 3000000))
					...
					if (isset($avatar))
					{
						...
						{
							$query = $db->prepare("UPDATE users SET name='$name',lastname='$lastname',yearB='$yearB',monthB='$monthB',dayB='$dayB',avatar='$avatar',email='$email' WHERE id='$id'");
							$res = $query->execute();
						}
					}
					else
					{
						...
						{
							$query = $db->prepare("UPDATE users SET name='$name',lastname='$lastname',yearB='$yearB',monthB='$monthB',dayB='$dayB',email='$email' WHERE id='$id'");
							$res = $query->execute();
						}
					}
					if ($res)
